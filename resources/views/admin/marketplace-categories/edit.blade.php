...
<div class="space-y-6">
    ...
    <div class="bg-[#1F1F1F] border border-[#282828] rounded-xl p-8">
        <form action="{{ route('admin.marketplace-categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <p class="text-gray-400 text-xs mt-1">Leave empty for main category, or select a parent for subcategory</p>
                    </div>

                    <div>
                        <label class="block text-white font-semibold mb-2">Description</label>
                        <textarea name="description" rows="4"
                                  class="w-full bg-[#161616] border border-[#282828] text-white p-3 rounded-lg focus:border-[#FFD900] focus:outline-none">{{ old('description', $category->description) }}</textarea>
                    </div>

                    <div>
                        <label class="block text-white font-semibold mb-2">Category Image</label>
                        @if($category->image)
                            <img src="{{ $category->image_url }}" alt="{{ $category->name }}" class="w-32 h-32 object-cover rounded-lg mb-3">
                        @endif
            ...
            <div class="mt-8 pt-6 border-t border-[#282828] flex flex-col sm:flex-row gap-4 justify-end">
                <a href="{{ route('admin.marketplace-categories.index') }}" class="px-6 py-3 bg-[#282828] text-white rounded-lg hover:bg-[#353535] transition-colors text-center">
                    Cancel
                </a>
