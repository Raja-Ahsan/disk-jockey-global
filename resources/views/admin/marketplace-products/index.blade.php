...
@section('title', 'Manage Marketplace Products')
@section('page-title', 'Manage Marketplace Products')
@section('page-description', 'View and manage all marketplace products')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-bold text-white">Marketplace Products</h2>
            <p class="text-gray-400 text-sm">Manage your marketplace product catalog</p>
        </div>
        <a href="{{ route('admin.marketplace-products.create') }}" class="btn primary-button">
    ...
    <div class="bg-[#1F1F1F] border border-[#282828] rounded-xl p-6">
        <form method="GET" action="{{ route('admin.marketplace-products.index') }}" class="flex flex-col md:flex-row gap-4">
            <input type="text" 
                   name="search" 
            ...
            @if(request()->hasAny(['search', 'category']))
                <a href="{{ route('admin.marketplace-products.index') }}" class="btn secondary-button whitespace-nowrap">Clear</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-[#1F1F1F] border border-[#282828] rounded-xl overflow-hidden">
        <div class="p-6 border-b border-[#282828]">
            <h2 class="text-xl font-bold text-white">All Marketplace Products ({{ $products->total() }})</h2>
        </div>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-[#282828] hover:bg-[#282828] transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.marketplace-products.show', $product->id) }}" class="text-[#FFD900] hover:underline text-sm">View</a>
                                <a href="{{ route('admin.marketplace-products.edit', $product->id) }}" class="text-blue-400 hover:underline text-sm">Edit</a>
                                @if($product->deleted_at)
                                    <form action="{{ route('admin.marketplace-products.destroy', $product->id) }}" method="POST" class="inline" onsubmit="return confirm('Permanently delete this product?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-400 hover:underline text-sm">Delete</button>
                                    </form>
                                @else
                                    <form action="{{ route('admin.marketplace-products.destroy', $product->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure? This will soft delete the product.')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-400 hover:underline text-sm">Delete</button>
                                    </form>
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-gray-400">
                            No products found. <a href="{{ route('admin.marketplace-products.create') }}" class="text-[#FFD900] hover:underline">Create your first marketplace product</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
