...
@section('page-title', 'Edit Marketplace Product')
@section('page-description', 'Update marketplace product information')
...
<div class="space-y-6">
    ...
    <div class="bg-[#1F1F1F] border border-[#282828] rounded-xl p-8">
        <form action="{{ route('admin.marketplace-products.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="productForm">
            @csrf
            @method('PUT')
                ...
                <div id="variationsContainer" class="space-y-4">
                    @if($product->isVariable() && $product->variations->count() > 0)
                        @foreach($product->variations as $variation)
                            @include('admin.marketplace-products.partials.variation-item', ['variation' => $variation, 'index' => $variation->id])
                        @endforeach
                    @endif
                </div>
            </div>
