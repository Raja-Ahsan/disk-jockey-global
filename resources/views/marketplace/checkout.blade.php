...
<main class="min-h-screen bg-[#161616] py-12 px-6">
    <div class="container mx-auto max-w-4xl">
        ...
        <form id="checkout-form" method="POST" action="{{ route('marketplace.checkout.process') }}" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-[#1F1F1F] border border-[#282828] rounded-lg p-6 sticky top-6" data-aos="fade-left">
                        ...
                        <a href="{{ route('marketplace.cart.index') }}" class="block text-center text-gray-400 hover:text-[#FFD900] mt-4 text-sm">
                            ← Back to Cart
                        </a>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('checkout-form').addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('{{ route('marketplace.checkout.process') }}', {
                method: 'POST',
                headers: {
            ...
            if (paymentIntent && (paymentIntent.status === 'succeeded' || paymentIntent.status === 'requires_capture')) {
                ...
                try {
                    const confirmResponse = await fetch('{{ url('/marketplace/checkout/confirm') }}/' + data.orderId, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Accept': 'application/json',
