...
<main class="min-h-screen bg-[#161616] py-12 px-6">
    <div class="container mx-auto max-w-6xl">
        ...
        @php
            ...
            $cartDebug = session()->get('marketplace_cart', []);
        @endphp
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-4">
                    ...
                    <div class="flex flex-col md:flex-row gap-6">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-4 mb-4">
                                <form action="{{ route('marketplace.cart.update', $item['cart_key']) }}" method="POST" class="flex items-center gap-2">
                                    @csrf
                                    @method('PUT')
                                ...
                                <form action="{{ route('marketplace.cart.remove', $item['cart_key']) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                ...
                <div class="text-right">
                    <form action="{{ route('marketplace.cart.clear') }}" method="POST" class="inline">
                        @csrf
            ...
            <div class="lg:col-span-1">
                <div class="bg-[#1F1F1F] border border-[#282828] rounded-lg p-6 sticky top-6" data-aos="fade-left">
                    ...
                    @auth
                    <a href="{{ route('marketplace.checkout.index') }}" class="btn primary-button w-full text-center py-4">
                        Proceed to Checkout
                    </a>
                    @else
                    ...
                    <a href="{{ route('marketplace.index') }}" class="block text-center text-gray-400 hover:text-[#FFD900] mt-4 text-sm">
                        Continue Shopping →
                    </a>
                </div>
            </div>
        ...
        <div class="bg-[#1F1F1F] border border-[#282828] rounded-lg p-12 text-center" data-aos="fade-up">
            ...
            @endif
            <a href="{{ route('marketplace.index') }}" class="btn primary-button inline-block">
                Browse Products
            </a>
        </div>
        @endif
